...
const DataTableWithButtons = () => {
  ...
  const updatemodalopenclick = (e) => {
    ...
    const filterdata = data.filter((m) => m.pg_name === e.target.id)
  ...
  const columns = [
    ...
    //     {row.pg_status === "ACTIVE" ? <Badge style={{ marginLeft: 10 }} id={row.pg_name} color='success' onClick={updatemodalopenclick}>Update</Badge> : <Badge style={{ marginLeft: 10 }} id={row.pg_name} color='success' onClick={updatedummydata}>Update</Badge>}  
    //     </span>
    //   }
    // }
  ]
  ...
  const searchalldataHandler = (e) => {
    ...
    if (e.target.value.length) {
      const updatedData = data.filter(item => {
        const startsWith =
          item.pgName.startsWith(e.target.value) ||
          item.pgUuid.startsWith(e.target.value)
        // ||
        ...
        const includes =
          item.pgName.includes(e.target.value) ||
          item.pgUuid.includes(e.target.value)
        // ||
    ...
    <Fragment>
      <UILoader blocking={block}>
        ...
        <Datatablecomponent data={data} coloumnsprops={columns} refreshdata={updatedata}
          routename="PG List"
          isaddpgrequest={true}
          handleModal={handleModal} ExpandableTable={<ExpandableTable />}
          searchFilter={searchalldataHandler}
          isaddfilter searchValue={searchValue}
          exportdata={exportdata}
        />
      </UILoader>
      <PGUpdateModal PGuuidselect={PGuuidselect} PGnamealready={PGnamealready} open={updatepgmodal} handleModal={updatepghandleModal} disabledbtns={updatepgbtndisabled} parentCallback={updatepgdetailsHandler} />

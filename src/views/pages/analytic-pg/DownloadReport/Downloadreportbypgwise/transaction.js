...
const Home = () => {
    ...
    const [selectpgnameidvalue, setselectpgnameidvalue] = useState('Select')
    ...
    //         sortable: true,
    //         minWidth: '150px',
    //         cell: row => <span>{moment(row.created).format('DD-MM-YYYY hh:mm')}</span>
    //     },
    //     {
    //         name: 'Update',
    //         selector: 'updatedAt',
    //         sortable: true,
    //         minWidth: '150px',
    //         cell: row => <span>{moment(row.updatedAt).format('DD-MM-YYYY hh:mm')}</span>
    //     },
    //     {
    //         name: 'Report Type',
    //         selector: 'reportType',
    ...
    const columns = [
        ...
        },
        {
            ...
            sortable: true,
            minWidth: '150px',
            cell: row => <span>{row.reportParam2 === null ? null : moment(row.reportParam2).format('DD-MM-YYYY')}</span>
        },
        {
            name: 'Report Type',
            selector: 'reportType',
    ...
    const getallgeneratedreportHandler = () => {
        setBlock(true)
        requestsApidata.reportgetAllReportTrListreportName("TRANSACTION_REPORT_PAYIN_PG_WISE").then(res => {
    ...
    const downloadreportHandler = () => {
        // console.log('downloadreportHandler', startDate, endDate, selectpgnameidvalue)
        if (startDate === "" || endDate === "" || selectpgnameidvalue === "Select") {
            toast.warning('All field is required')
        } else {
            console.log('run', searchEndDate, searchStartDate, selectpgnameidvalue)
            setbtndisabled(true)
            requestsApidata.payoutgeneratertransactionReport("TRANSACTION_REPORT_PAYIN_PG_WISE", searchStartDate, searchEndDate, selectpgnameidvalue.value).then((res) => {
                if (res.data.successCode === "API_SUCCESS") {
        ...
        <Fragment>
            ...
            >
                ...
                <ModalBody className='flex-grow-1'>
                    <CardTitle tag='h4'>Generate Report PG Wise</CardTitle>
                    <div>
                        <Row>
                            <FormGroup tag={Col} md='12'>
                                <Label for='merchantid'>Select PG</Label>
                                <Select
                                    isClearable={false}
                                    id='kycStatus'
                ...
                <div style={{ marginBottom: 50 }}>
                    <UILoader blocking={block}>
                        ...
                        <Datatablecomponent data={allgetGeneratedreportlist} coloumnsprops={columns} refreshdata={updatedata}
                            routename="Transaction Report PG Wise"

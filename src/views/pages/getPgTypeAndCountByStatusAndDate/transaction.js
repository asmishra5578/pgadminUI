...
const Home = () => {
    ...
    const updatedata = async () => {
        ...
        setBlock(true)
        requestsApidata.getPgTypeAndCountByStatusAndDate(fromDate, toDate, "SUCCESS").then(res => {
            if (res.data.exception === "IDEAL_SESSION_EXPIRED") {
                ...
                res.data.map((v) => {
                    v.datefrom = startDate === "" ? fromDate : searchStartDate
                    v.todate = endDate === "" ? toDate : searchEndDate
    ...
    const filteByDate = () => {
        ...
        requestsApidata.getPgTypeAndCountByStatusAndDate(startDate === "" ? fromDate : searchStartDate, endDate === "" ? toDate : searchEndDate, selectkycstatusvalue === "Select" ? "SUCCESS" : selectkycstatusvalue.value).then(res => {
            if (res.data.exception === "IDEAL_SESSION_EXPIRED") {
                toast.error('Session Expired Login again!')
                history.push('/')
            } else if (res.data.exception === "JWT_MISSING") {
        ...
        <Fragment>
            ...
            <div>
                <UILoader blocking={block}>
                    <Datatablecomponent coloumnsprops={columns} data={data}
                        // isaddfilter
                        searchValue={searchValue} searchFilter={searchalldataHandler} routename="Transaction Report PG Type and Count by Status and Date" />
                </UILoader>
            </div>
        </Fragment>
    )

...
const Home = () => {
    ...
    const getallgeneratedreportHandler = () => {
        setBlock(true)
        requestsApidata.reportgetAllReportTrListreportName("WALLET_TRANSACTION_REPORT_PAYOUT_MERCHANT_WISE").then(res => {
    ...
    const getallpayoutmerchantlis = () => {
        requestsApidata.findAllPayoutMerchant().then(res => {
            setpayoutmerchant(res.data)
        }).catch((err) => {
    ...
    const downloadreportHandler = () => {
        // console.log('downloadreportHandler', startDate, endDate, selectmerchantidvalue)
        if (startDate === "" || endDate === "" || selectmerchantidvalue === "Select") {
            toast.warning('All field is required')
        } else {
            // console.log('run', searchEndDate, searchStartDate, merchantidvalue)
            setbtndisabled(true)
            requestsApidata.payoutgeneratertransactionReport("WALLET_TRANSACTION_REPORT_PAYOUT_MERCHANT_WISE", searchStartDate, searchEndDate, selectmerchantidvalue.value).then((res) => {
        ...
        <Fragment>
            ...
            >
                ...
                <ModalBody className='flex-grow-1'>
                    <CardTitle tag='h4'>Generate Wallet Report Merchant Wise</CardTitle>
                    <div>
                        <Row>
                ...
                <div style={{ marginBottom: 50 }}>
                    <UILoader blocking={block}>
                        ...
                        <Datatablecomponent data={allgetGeneratedreportlist} coloumnsprops={columns} refreshdata={updatedata}
                            routename="Wallet Transaction Report Merchant Wise"

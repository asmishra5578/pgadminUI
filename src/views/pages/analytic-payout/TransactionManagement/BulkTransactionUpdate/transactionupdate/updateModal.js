...
const accountTransfer = ({ open, upihandlerModal, parentaccounttra, disabledaccountbtn, displayACCOUNTloading }) => {
  const SignupSchema = yup.object().shape({
    internalOrderId: yup.string().required('Internal Order ID is required'),
    utrid: yup.string(),
            ...
            {dataarray.map((v, k) => { 
              return <tr>
                ...
                <span style={{ cursor: 'pointer', color:'green' }} id={v.internalOrderId} onClick={editDataHandler}>Edit</span>
                  ...
                  {/* <Edit2 style={{ cursor: 'pointer' }} id={v.internalOrderId} onClick={editDataHandler} /> <Trash2 onClick={deleteDataHandler} id={v.internalOrderId} style={{ marginLeft: 10, cursor: 'pointer' }} /> */}
                </td>
              </tr>
            })}
          </table>
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
            <Row>
              <FormGroup tag={Col} md='2'>
                <Label for='internalOrderId'>Internal Order ID</Label>
                <Input
                  type='text'
                  id='internalOrderId'

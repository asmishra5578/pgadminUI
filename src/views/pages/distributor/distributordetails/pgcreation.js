...
const createpg = ({ open, handleModal, parentCallback, disabledbtns }) => {
  ...
  const SignupSchema = yup.object().shape({
    ...
    distributorName: yup.string().required('Distributor Name can not be empty').matches((/^([a-zA-Z]+\s)*[a-zA-Z]+$/), 'Name is not valid'),
    distributorEmailId: yup.string().required('Email is required').matches(/^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/, 'Email not valid formate'),
    companyName: yup.string().required('Company Name can not be empty'),
    distributorType: yup.string().required('Distributor Type can not be empty'),
  ...
  return (
    ...
    >
      ...
      <ModalBody className='flex-grow-1'>
        <CardTitle tag='h4'>Configuration Distributor</CardTitle>
        <Form onSubmit={handleSubmit(onSubmit)}>
        <Row>
          <FormGroup tag={Col} md='12'>
            <Label className='form-label' for="distributorName">
              Distributor Name<span style={{color:"red", fontSize:14, marginTop:-10}}>*</span>
            </Label>
            <Input
              ...
              invalid={errors.distributorName && true}
            />
            {errors && errors.distributorName && <FormFeedback>{errors.distributorName.message}</FormFeedback>}
          </FormGroup>
          <FormGroup tag={Col} md='12'>
            ...
            <Input
              ...
              id='distributorEmailId'
              placeholder='john.doe@email.com'
              innerRef={register({ required: true })}
              invalid={errors.distributorEmailId && true}
            />
            {errors && errors.distributorEmailId && <FormFeedback>{errors.distributorEmailId.message}</FormFeedback>}
          </FormGroup>
        </Row>
        <Row>
          <FormGroup tag={Col} md='12'>
        ...
        <Row>
          <FormGroup tag={Col} md='12'>
            <Label className='form-label' for={`distributorType`}>
              Distributor Type<span style={{color:"red", fontSize:14, marginTop:-10}}>*</span>
            </Label>
            <Input
              ...
              innerRef={register({ required: true })}
              invalid={errors.distributorType && true}
            />
            {errors && errors.distributorType && <FormFeedback>{errors.distributorType.message}</FormFeedback>}
          </FormGroup>
          <FormGroup tag={Col} md='12'>
